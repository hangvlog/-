        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(saleAttr,index) in spuSaleAttrList"
                  :key="saleAttr.id">
                <dt class="title">{{ saleAttr.saleAttrName }}</dt>
                <dd changepirce="0"
                    :class="saleAttrItem.isChecked === '1'?'active':''"
                    v-for="(saleAttrItem,index) in saleAttr.spuSaleAttrValueList"
                    :key="saleAttrItem.id"
                    @click="changeActive(saleAttrItem,saleAttr.spuSaleAttrValueList)">{{ saleAttrItem.saleAttrValueName }}</dd>
...
export default {
  ...
  methods: {
    ...
    changeActive (saleAttrItem, arr) {
      // 先排他让所有元素都不被选择
      arr.forEach(item => item.isChecked = '0')
      // 再让当前元素被选择
      saleAttrItem.isChecked = '1';
    },
    changeCount (e) {
      console.log(e)
      if (isNaN(this.count * 1)) this.count = '1'
      // else this.count = parseInt(Math.abs(this.count))
    },
    controllerInput (e) {
      // console.log(e);
      // 如果输入的不符合条件
  ...
  computed: {
    ...mapState({
      price: state => state.detail.goodsInfo.price
    }),
    ...mapGetters(['categoryView', 'skuInfo', 'spuSaleAttrList']),
    SkuNumber () {
      if (eval(this.count) == 'NaN' || this.count == 'NaN') {
        return '';
      } else {
        return this.count
      }
